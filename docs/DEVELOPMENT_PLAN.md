...
- [x] **Task 3.3**: Network Graph Builder
  ...
  - [x] Role classification for outreach leverage (`talent`, `engineering_lead`, `peer_engineer`).
- [x] **Task 3.5**: Company Evaluation & Clustering
  - [x] Composite company scoring (Job Fit 45% / Domain Fit 35% / Network 20%).
  - [x] Action clusters (Warm Outreach, Direct Apply, Network Nurture, Watchlist) with recommended actions.
  - [x] Domain cluster classification (GenAI/LLMs, Core ML/CV, Enterprise AI, FinTech, Healthcare, General).
  - [x] `companies` table in SQLite + `evaluate-companies` / `companies` / `company` CLI commands.
  - [x] Enriched Company notes and `00_Company_Clusters.md` hub note in the vault.
