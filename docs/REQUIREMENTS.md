...
- **Job Search**: Automated search for relevant job postings based on the user's profile.
  - _Source_: Primarily LinkedIn, potentially other boards (Indeed, Glassdoor).
  - _Filters_: Location, Remote/Hybrid, Salary, Company Size.
...
- **Role Classification**: Tag connections by outreach leverage — `talent` (recruiters/sourcers), `engineering_lead` (managers/directors/CTOs), `peer_engineer` (AI/ML/software engineers), or `other`.

### 2b. Company Evaluation & Outreach Prioritization

- **Company Scoring**: Score every company with open jobs or network connections on a 0-100 composite: Job Opportunity Quality (45%), Company Domain Fit (35%), and Network Leverage (20%).
- **Action Clusters**: Assign each company a strategic action tier — `Warm Outreach` (referral priority), `Direct Apply`, `Network Nurture`, or `Watchlist` — each with a concrete recommended next step.
- **Domain Clusters**: Classify companies by tech domain (e.g. `Generative AI & LLMs`, `Enterprise AI & Cloud Platforms`) to spot where your profile fits best.
- **Company Dossier**: Persist evaluations to the database and render them into Obsidian company notes (score breakdown, action checklist, scored jobs with apply links, role-tagged contacts) plus a `00_Company_Clusters.md` hub note.
